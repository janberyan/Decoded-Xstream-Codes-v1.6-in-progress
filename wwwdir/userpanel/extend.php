...
require (IPTV_TEMPLATES_PATH . '/' . '/userpanel/' . 'extend.php');
...
default:
	...
	$Cecfa885eed4e0be5ea572f6a5e134a4 = c24a41ed39F4Be3316A3ECb1884c6b23(array('member_id' => $c83b7d347a6bc7f5f0e3313147e88ecc->member_info['id'], 'is_mag' => 0));
	...
	$Fce20624c9b6567a7a8c441a4651444d = $f9a8823715bcb54e8069e0086da240b2['you_have_no_lines'];

	$b8980526936694b2cd4cf6c7f8c80396 = B8a6ea05eEbc7E2e3503ea794B1a1ee4::$request['action'];
	session_start();
case 'extend':
	$D29f3c5494cb370b46f3e907dc61f106->query('INSERT INTO `reg_userlog` (`owner`,`username`,`password`,`date`,`type`) VALUES(\'%s\',\'%s\',\'%s\',\'%s\',\'%s\')', $c83b7d347a6bc7f5f0e3313147e88ecc->member_info['username'], $deb595af0f0f430af9497132ddb4247a['username'], $deb595af0f0f430af9497132ddb4247a['password'], time(), $f9a8823715bcb54e8069e0086da240b2['extend_line'] . ' ' . $Fac708114f97b667403407d22b554e84 . ' [ ' . $A0951fe8d1e4d37a4b8831a6eebd8a54['package_name'] . ' | (exp ' . date('d/m/Y H:i:s', $fcbd8a94125e8f5a21433cd1ab90e407) . ') ]');
	...
	default:
		...
		if ((0 < count(b8A6ea05Eebc7E2e3503ea794b1A1ee4::$request)) && !empty(B8a6Ea05EeBC7E2e3503eA794B1A1Ee4::$request['user_id']) && !empty(b8A6ea05eeBc7E2E3503eA794B1a1ee4::$request['package_id']) && !empty(B8a6EA05Eebc7e2e3503ea794B1a1EE4::$request['line_type']) && (b8A6Ea05eebC7E2e3503Ea794B1a1Ee4::$request['line_type'] == 'official')) {
		}
		...
		while (!(!empty($A0951fe8d1e4d37a4b8831a6eebd8a54) && ($A0951fe8d1e4d37a4b8831a6eebd8a54['is_official'] == 1) && ($A0951fe8d1e4d37a4b8831a6eebd8a54['only_mag'] == 0))) {
		}

		$D29f3c5494cb370b46f3e907dc61f106->query('DELETE FROM `user_output` WHERE `user_id` = \'%d\'', $fd7f2f8cde8b865243a388c219894e64);
		while (!($deb595af0f0f430af9497132ddb4247a['exp_date'] <= time())) {
		}
